...
class config_vis(object):
    def __init__(self, database, expID, configForm, standardize):
        ...
        for scn in solverConfigName:
            ...
            if page > 0:
                if count >= int(minDict['confidence']) and count <= int(maxDict['confidence']) and float(
                        parameterValue[scn]['performance']) >= float(minDict['performance']) and float(
                        parameterValue[scn]['performance']) <= float(maxDict['performance']):
                    choosenConfigs.append(scn)
                else:
                    deselectedConfigs.append(scn)

        if page > 0:
            configurable = [p.Parameters_idParameter for p in experiment.configuration_scenario.parameters if
            ...
            paramInstance = db.session.query(db.ParameterInstance).filter(
                db.ParameterInstance.SolverConfig_idSolverConfig.in_(choosenConfigs)).all()
        ...
        for pl in paramList:
            ...
            if domain[pl] == 'num':
                if page > 1 and len(minDict[pl]) > 0 and float(minDict[pl]) >= min(valueList) and float(
                ...
                if page > 1 and len(maxDict[pl]) > 0 and float(maxDict[pl]) >= min(valueList) and float(
                ...
                if standardize == 1:
                    ...
                    if math.fabs(min(values)) > max(values):
                        values = [(v - min(values)) / max(values) for v in
                ...
                paramAttribute[i] = {'values': values, 'min': minValue, 'max': maxValue, 'name': parameterName[pl],
                                     'id': pl, 'hide': hide, 'turn': turn, 'positionList': positionList,
                                     'domain': 'num'}

            elif domain[pl] == 'cat':
                values = project(values, max(values))
                paramAttribute[i] = {'values': values, 'valueList': valueList, 'selectValueList': selectValueList[pl],
                                     'name': parameterName[pl], 'id': pl, 'hide': hide, 'turn': turn,
                                     'positionList': positionList, 'domain': 'cat'}
        if standardize == 1:
            for ri in range(i):
