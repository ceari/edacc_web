    ...
    def __init__(self, database, status, expID, **config):
        ...
        if status == ['pm']: 
            ...
            for t, s in zip(problemText, problemStatus):
                ...
                self.create_rectangle(winWidth - 115, y1, winWidth - 105, y2, width = 1, fill = constants.JOB_STATUS_COLOR[s])
                y1 = y1 + 26
                y2 = y2 + 26
        else:
            ...
            for c in status:
                ...
                self.create_rectangle(winWidth - 115, y1, winWidth - 105, y2, width = 1, fill = constants.JOB_STATUS_COLOR[c])
                y1 = y1 + 26
                y2 = y2 + 26
        ...
        for c in cluster:
            ...
            clusterImageMap.append({"id": location, "position": clusterCenter, "clients": clientImageMap, "radius": radiusDiagram}) 
