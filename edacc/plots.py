...
def result_property_comparison(results1, results2, solver1, solver2, result_property_name, log_property, dim=700,
    ...
    if format == "rscript":
        file.write(
            "plot(c(), c(), type='p', col='red', las=1, log='%s', xlim=c(%f, %f), ylim=c(-0.05, 1.05), xaxs='i', yaxs='i', xlab='', ylab='', cex.main=1.5)\n" \
            % (log, min_x, max_x))
...
def property_distributions(results, property_name, log_property, filename, format='png'):
    ...
    for res in results:
        if len(res[1]) > 0:
            ...
            if format == "rscript":
                file.write(
                    "ecdf(c(%s), main='', col='%s', pch=%d, log='%s', xlab='', ylab='', xaxs='i', yaxs='i', las=1, xaxt='n', yaxt='n', xlim=c(%f, %f), ylim=c(-0.05, 1.05))\n" \
...
def property_distribution(results_by_sc, property_name, log_property, restart_strategy, filename, format='png'):
    ...
    if format == "rscript":
        file.write(
            "plot(ecdf(c(%s)), main='', xaxt='n', yaxt='n', log='%s', xlab='', ylab='', xaxs='i', yaxs='i', las=1, xlim=c(%f, %f), ylim=c(-0.05, 1.05))\n" \
...
@synchronized
def runtime_matrix_plot(flattened_rtmatrix, num_sorted_solver_configs, num_sorted_instances, measure, filename,
                        format='png'):
    if format == 'png':
    ...
    robjects.r.image(robjects.IntVector(range(1, num_sorted_instances + 1)),
                     robjects.IntVector(range(1, num_sorted_solver_configs + 1)),
    ...
    if format == "rscript":
        ...
        file.write("image(c(%s), c(%s), m, col=color_ramp, xlab='', ylab='', ylim=c(%d, 1), zlim=c(%f, %f))\n" \
                   % (','.join(map(str, range(1, num_sorted_instances + 1))),
                      ','.join(map(str, range(1, num_sorted_solver_configs + 1))),
    ...
    robjects.r.image(1, color_levels,
                     robjects.r.matrix(data=color_levels, ncol=robjects.r.length(color_levels), nrow=1),
    ...
    if format == "rscript":
        ...
        file.write(
            "image(1, color_levels, matrix(data=color_levels, ncol=length(color_levels), nrow=1), col=color_ramp, xlab='', ylab='', xaxt='n')\n")
        file.write("layout(1)\n")
...
def parameter_plot_1d(data, parameter_name, measure, runtime_cap, log_x, log_y, filename, format='png'):
    ...
    try:
        ...
        robjects.r.plot(robjects.FloatVector(xs), robjects.FloatVector(ys), log=log,
                        type='p', col=colors[col % len(colors)], las=1,
                        main=measure + ' runtime against ' + parameter_name,
                        xlim=robjects.FloatVector([min(xs), max(xs)]),
                        ylim=robjects.FloatVector([0.0 if not log_y else 0.000000001, max(ys)]),
...
@synchronized
def parameter_plot_2d(data, parameter1_name, parameter2_name, measure, surface_interpolation, runtime_cap, log_x, log_y,
                      log_cost, filename, format='png'):
    if format == 'png':
        grdevices.png(file=filename, units="px", width=1000,
                      height=800, type="cairo")
    elif format == 'pdf':
    ...
    try:
        ...
        if log_cost: min_cost_point = min_cost_point[0], min_cost_point[1], 0.0 if min_cost_point[
                                                                                       2] <= 0.000000001 else math.log10(
            min_cost_point[2])
        ...
        if not surface_interpolation:
            ...
            title = "Minimum: (%s, %s) with cost: %s" % (
            round(min_cost_point[0], 4), round(min_cost_point[1], 4), round(min_cost_point[2], 4))

            if format == 'rscript':
                file.write("cols = heat.colors(256)[c(" + ','.join(map(str, costs)) + ")]\n")
                file.write(
                    "plot(c(%s), c(%s), log='%s', type='p', col=cols, pch=19, xlab='%s', ylab='%s', xaxs='i', yaxs='i', cex=1.5, xlim=c(%f, %f), ylim=c(%f, %f), main='%s')\n" \
                    % (','.join(map(str, xs)), ','.join(map(str, ys)), log, parameter1_name, parameter2_name, min(xs),
                       max(xs), min(ys), max(ys), title))
                file.write("par(new=1)\n")
            ...
            robjects.r.plot(robjects.FloatVector(xs), robjects.FloatVector(ys), log=log, type='p', col=cols, pch=19,
                            ...
                            xlim=robjects.FloatVector([min(xs), max(xs)]),
                            ylim=robjects.FloatVector([min(ys), max(ys)]),
                            main=title)
            robjects.r.par(new=1)
            robjects.r.points(robjects.FloatVector([min_cost_point[0]]), robjects.FloatVector([min_cost_point[1]]),
            ...
            surf = akima.interp(robjects.FloatVector(xs), robjects.FloatVector(ys), robjects.FloatVector(costs),
                                xo=robjects.r.seq(min_x, max_x, (max_x - min_x) / 1000.0),
                                yo=robjects.r.seq(min_y, max_y, (max_y - min_y) / 800.0),
                                duplicate="mean")
            title = "Minimum: (%s, %s) with cost: %s" % (
            round(min_cost_point[0], 4), round(min_cost_point[1], 4), round(min_cost_point[2], 4))
            ...
            if format == 'rscript':
                file.write(
                    "surf = interp(c(%s), c(%s), c(%s), xo=seq(%f, %f, %f), yo=seq(%f, %f, %f), duplicate='mean')\n" \
                    % (','.join(map(str, xs)), ','.join(map(str, ys)), ','.join(map(str, costs)), min_x, max_x,
                       (max_x - min_x) / 1000.0, min_y, max_y, (max_y - min_y) / 800.0))
                file.write(
                    "image.plot(surf, nlevel=256, xlab='%s', ylab='%s', xaxs='i', yaxs='i', xlim=c(%f, %f), ylim=c(%f, %f), main='%s', legend.lab='%s', legend.mar=4.5)\n" \
            ...
            robjects.r("image.plot")(surf, nlevel=256,
                                     ...
                                     xlim=robjects.FloatVector([min_x, max_x]),
                                     ylim=robjects.FloatVector([min_y, max_y]),
                                     main=title,
                                     **{'legend.lab': measure, 'legend.mar': 4.5})
            robjects.r.par(new=1)
            robjects.r.points(robjects.FloatVector([min_cost_point[0]]), robjects.FloatVector([min_cost_point[1]]),
                              type='p', pch=3, col='red', cex=2)
    except Exception as ex:
        raise ex
    finally:
        grdevices.dev_off()
...
def perc_solved_alone(perc_solved_by_solver, filename, format='png'):
    ...
    try:
        ...
        robjects.r.barplot(robjects.FloatVector([t[1] for t in data]), horiz=True,
                           names=robjects.StrVector([t[0] for t in data]), las=1, xlim=robjects.FloatVector([0.0, 1.0]))
    except Exception as ex:
        raise ex
    finally:
        grdevices.dev_off()
